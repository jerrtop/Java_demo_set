			...
			for(var name in data){
				...
				if (!rr.length){
					...
					if (f.length){
						...
						//begin edit by jerry at 2013/9/3 11:16
						//checkbox mulit checked
						$("input[name='"+name+"'][value='"+val+"']:radio,input[name='"+name+"'][value='"+val+"']:checkbox",form).attr('checked', true);
						if(typeof val=='object'){
							var cbSelector='';
							for(var i=0; i < val.length - 1 ; i++){
								var cbSelector = $("input[name='"+name+"'][value='"+val[i]+"']:checkbox");
								if(cbSelector!='' && cbSelector.length > 0){
									$(cbSelector.substring(1),form).attr('checked',true);
								}
							}
						}
						//end
